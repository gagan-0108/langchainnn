...
from pydantic import typing, Field , EmailStr , BaseModel

load_dotenv()

model = ChatGoogleGenerativeAI(model = "gemini-2.5-flash")
class Review(BaseModel):
    key_themes: list[str] = Field(description="Write down all the key point from the review")
    summary: str = Field( description= "A brief summary of a review")
    sentiment: Literal["pros", "cons"] = Field(description="Return sentiment of the review either negative, positive or neutral")
    pros: Optional[list[str]]=Field( description="Write down all the pros inside tk",default = None )
    cons: Optional[list[str]] = Field(description="Write down all the cons inside it", default=None )
